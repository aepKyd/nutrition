...
from app.services import ingredient_synonyms as ingredient_synonym_service
...
def get_ingredient_synonyms(ingredient_id: int, conn: psycopg2.extensions.connection = Depends(get_db_connection)):
    try:
        return ingredient_synonym_service.get_ingredient_synonyms(conn, ingredient_id)
...
def create_ingredient_synonym(ingredient_id: int, synonym: IngredientSynonymCreate, conn: psycopg2.extensions.connection = Depends(get_db_connection)):
    try:
        new_synonym = ingredient_synonym_service.create_ingredient_synonym(conn, ingredient_id, synonym)
        conn.commit()
        return new_synonym
    except psycopg2.Error as e:
        conn.rollback()
...
def delete_ingredient_synonym(ingredient_id: int, synonym_id: int, conn: psycopg2.extensions.connection = Depends(get_db_connection)):
    try:
        deleted_count = ingredient_synonym_service.delete_ingredient_synonym(conn, ingredient_id, synonym_id)
        if deleted_count == 0:
